...
from {{ ref('ins_auto_pop_wt_new_premiums_full_inserted') }} a_1
...
LEFT JOIN ins_auto_scores_2023 r 
	ON btrim(a_1.state_code::text) = btrim(r.state::text) AND btrim(a_1.provider::text) = btrim(r.provider::text)
