...
WITH cte_1 AS (
         SELECT TO_CHAR(ins_auto_master_table_2022.data_date, 'MM/DD/YYYY') as data_date,
           ...
           FROM {{ ref('ins_auto_master_table_state_2023_temp_full') }} ins_auto_master_table_2022
          WHERE  ins_auto_master_table_2022.age::text = '40'::text AND ins_auto_master_table_2022.vehicles::text = '2012 Toyota Camry LE'::text AND (ins_auto_master_table_2022.insurancescore_alignment::text = ANY (ARRAY['Blank'::character varying::text, 'Good'::character varying::text])) AND ins_auto_master_table_2022.driving_record_violations::text = 'Clean'::text AND ins_auto_master_table_2022.is_user_a_veteran = 'No'::text AND ins_auto_master_table_2022.bi_pd_limit = 'State Minimum'::text
         ...
         SELECT cte_1.data_date,
            ...
            cte_1.state_pop_wt_rate_mandatory + cte_1.state_pop_wt_rate_bi_pd + cte_1.state_pop_wt_rate_umbi + cte_1.state_pop_wt_rate_fees AS annual_premium,
            (cte_1.state_pop_wt_rate_mandatory + cte_1.state_pop_wt_rate_bi_pd + cte_1.state_pop_wt_rate_umbi + cte_1.state_pop_wt_rate_fees) / 12::numeric AS monthly_premium
           FROM cte_1
        ), cte_min AS (
         ...
         SELECT a.data_date,
            ...
            a.annual_premium,
            a.monthly_premium,
            (0.05 * a.stability_score + 0.25 * a.satisfaction_score + 0.20 * a.claims_score + 0.10 * a.coverage_score + 0.40 * a.affordability_score) * 20::numeric AS mg_total_best_100
           FROM cte_3 a
        ),cte_scores as 
			(
			select 
         ...
         SELECT a.data_date,
            ...
            a.monthly_premium,
            a.mg_total_best_100,
			(((1-0.60) * (a.mg_total_best_100)/100) +0.60)*100 as mg_total_best_100_normalized,
 ...
 SELECT cte_best.data_date,
	...
	case when (max_satisfaction_score-min_satisfaction_score) = 0 then 25
				else 
				((satisfaction_score - cte_scores.min_satisfaction_score)/(cte_scores.max_satisfaction_score - cte_scores.min_satisfaction_score) * (25-9))+9 end as satisfaction_score_25,
	...
	case when (max_affordability_score-min_affordability_score) = 0 then 40
				else 
					((affordability_score - cte_scores.min_affordability_score)/(cte_scores.max_affordability_score - cte_scores.min_affordability_score) * (40-12))+12 end as affordability_score_40

   FROM cte_best
   left join cte_scores on cte_best.state = cte_scores.state and cte_best.data_date = cte_scores.data_date
   ),
	   cte_rankings as (
   select *,
      		...
      		dense_rank() OVER (PARTITION BY data_date, state, age,coverage ORDER BY satisfaction_score_25 DESC NULLS LAST)::varchar AS satisfaction_rank,
     		...
     		dense_rank() OVER (PARTITION BY data_date, state, age,coverage ORDER BY affordability_score_40 DESC NULLS LAST)::varchar AS affordability_rank,
   		    round((stability_score_05 +  satisfaction_score_25 + claims_score_20 +coverage_score_10 + affordability_score_40),2)::varchar as mg_total_best_100_normalized ,
		    dense_rank() OVER (PARTITION BY data_date, state, age, coverage ORDER BY stability_score_05 + 
							  satisfaction_score_25 + claims_score_20 +coverage_score_10 + affordability_score_40 DESC NULLS LAST)::varchar AS rank_state_best		   
   from cte_final) 
   select data_date,
   			state,
			age,
			...
			monthly_premium,
			stability_score_05,
			satisfaction_score_25,
			claims_score_20,
			coverage_score_10,
			affordability_score_40,
			...
			case when satisfaction_score_25 is null then 'No Score'
			...
			case when affordability_score_40 is null then 'No Score'
